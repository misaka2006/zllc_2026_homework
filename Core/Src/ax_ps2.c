...
#include "ax_ps2.h"
#include "core_cm3.h"  // 包含CoreDebug和DWT定义
...
void AX_PS2_ScanKey(JOYSTICK_TypeDef *JoystickStruct)
{
	...
	// 只有在有效模式下才更新按键和摇杆数据
	if(PS2_data[1] == 0x73 || PS2_data[1] == 0x41)
	{
	...
	{
		// 无效模式下保持默认值
		JoystickStruct->btn1 = 0;
		JoystickStruct->btn2 = 0;
		JoystickStruct->RJoy_LR = 127;
		JoystickStruct->RJoy_UD = 127;
		JoystickStruct->LJoy_LR = 127;
		JoystickStruct->LJoy_UD = 127;
	}
}
