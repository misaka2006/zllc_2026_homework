...
{
    if (data[SERVO_CONTROL_SIZE + 1] == crc8_table_driven(data, SERVO_CONTROL_SIZE + 1)) {
        control->servo1 = data[1];
        control->servo2 = data[2];
        control->servo3 = data[3];
        control->servo4 = data[4];
    }
}
...
void CarStateDataInit(Car_State_Data *control, uint8_t *data)
{
    ...
    // 舵机数据
    data[13] = control->servo1;
    data[14] = control->servo2;
    data[15] = control->servo3;
    data[16] = control->servo4;

    // 运动状态数据
    memcpy(data + 1, &(control->Vx), 4);
    memcpy(data + 5, &(control->Vy), 4);
    memcpy(data + 9, &(control->Vz), 4);
    EndianTransfer(data + 1, 4);
    EndianTransfer(data + 5, 4);
    EndianTransfer(data + 9, 4);
