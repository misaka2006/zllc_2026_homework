...
// 连接状态检测
uint8_t PS2_Check_Connection(JOYSTICK_TypeDef *js)
{
    // 检查数据是否合理
    if(js->mode == 0x00 || js->mode == 0xFF) return 0;
    if(js->btn1 == 0xFF && js->btn2 == 0xFF) return 0;
    return 1;
}
 extern JOYSTICK_TypeDef joystick;
// 自动重连
void PS2_Auto_Reconnect(void)
{
    ...
    if(HAL_GetTick() - last_check > 1000) { // 每秒检查一次
        ...
        if(!PS2_Check_Connection(&joystick)) {
            error_count++;
            if(error_count > 3) { // 连续3次检测失败
                // 重新初始化PS2
               AX_PS2_Init(); 
                error_count = 0;
            }
